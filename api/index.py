...
# --- FastAPI app (redirect_slashes=False pour eviter 307) ---
...
async def oauth_authorize_get(response_type: str = "code", client_id: str = "", redirect_uri: str = "", scope: str = "mcp", state: str = "", code_challenge: str = "", code_challenge_method: str = "S256"):
    html = f"""<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>Legifrance MCP - Autorisation</title>
    <style>body{{font-family:system-ui,sans-serif;max-width:420px;margin:80px auto;padding:24px;text-align:center;background:#f9f9f9}}
    ...
    .allow{{background:#0070f3;color:#fff}}.deny{{background:#e00;color:#fff}}</style></head>
    <body><div class="card">
    <h2>&#127467;&#127479; Legifrance MCP</h2>
    <p><strong>{client_id}</strong> demande l'acces au serveur MCP Legifrance (droit francais).</p>
...
# Vercel utilise directement l'objet ASGI 'app'
