...
def _post_legifrance(path: str, payload: dict[str, Any]) -> Any:
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Legifrance error: {exc}" + (f" | {body}" if body else ""),
        ) from exc
...
def rechercher_dans_texte_legal(
    ...
    type_recherche: Literal["TOUS_LES_MOTS_DANS_UN_CHAMP", "EXPRESSION_EXACTE", "AU_MOINS_UN_MOT"] | None = None,
...
# --- FastAPI app ---
# redirect_slashes=False evite le 307 /mcp -> /mcp/ qui casse Mangum
app = FastAPI(title="Legifrance MCP", version="1.0.0", redirect_slashes=False)

# Monte le serveur MCP sous /mcp
...
async def oauth_authorize_get(
    ...
    <title>Legifrance MCP - Autorisation</title>
    <style>
      body{{font-family:system-ui,sans-serif;max-width:440px;margin:80px auto;padding:24px;background:#f5f5f5}}
      .card{{background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 16px rgba(0,0,0,.1);text-align:center}}
      h2{{margin-bottom:8px;color:#111}}p{{color:#555;margin-bottom:28px;line-height:1.5}}
      button{{padding:12px 28px;border-radius:8px;border:none;font-size:15px;cursor:pointer;margin:6px;font-weight:600}}
      .allow{{background:#0070f3;color:#fff}}.deny{{background:#dc2626;color:#fff}}
    </style></head>
    <body><div class="card">
      <h2>&#127467;&#127479; Legifrance MCP</h2>
      <p><strong>{client_id}</strong><br>demande l'acces au serveur MCP Legifrance<br>(droit francais, jurisprudence, codes).</p>
...
def health():
    return {
        ...
        "legifrance_base_url": LEGIFRANCE_BASE_URL or "NOT SET",
    }


# Vercel handler — api_gateway_base_path="/" + strip_stage_path=True
# resout le 404 sur les sous-apps montees avec Mangum
